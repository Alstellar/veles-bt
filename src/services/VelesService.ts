// src/services/VelesService.ts

import type { BacktestResultItem } from '../types'; // Общие типы
import type { 
  VelesConfigPayload, 
  BacktestStatusResponse, 
  BacktestStats, 
  UserProfile 
} from '../types/veles'; // Наши новые типы
...
} from './VelesInjections'; // Импорт инъекций
...
export class VelesService {
  ...
  static async runTest(tabId: number, token: string, payload: VelesConfigPayload): Promise<{ success: boolean, status: number, id?: number, error?: string }> {
    ...
    return { success: false, status: res?.status || 0, error: res?.error || JSON.stringify(res?.body) };
  }
